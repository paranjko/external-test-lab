...
lock="$root/.gdc-identity-layout.lock"; exec 9>"$lock"; flock -n 9 || { echo 'another identity layout migration is in progress' >&2; exit 1; }
...
stage="$(mktemp -d "$root/.gdc-identity-layout.XXXXXX")"; trap 'rm -rf -- "$stage"' EXIT
