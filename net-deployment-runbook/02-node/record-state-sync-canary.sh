...
' "$receipt" >/dev/null || { echo 'lineage_verification_failed: receipt is not a pending two-provider P2P canary contract' >&2; exit 1; }
status="$(docker compose --env-file "$deploy/.env" -f "$deploy/compose.yaml" exec -T node sh -c 'curl -fsS http://127.0.0.1:26657/status')" \
