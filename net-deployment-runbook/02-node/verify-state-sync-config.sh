...
config="$(docker compose --env-file "$deploy/.env" -f "$deploy/compose.yaml" exec -T node sh -c 'cat /root/.inference/config/config.toml')" || { echo 'lineage_verification_failed: cannot read canary config.toml' >&2; exit 1; }
