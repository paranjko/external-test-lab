...
usage() { echo "Usage: $0 --inventory FILE --node-name SSH_ALIAS --account-public FILE [--seeds-file FILE|--bootstrap] --secrets-dir DIR [--poc-callback-url URL --ml-callback-bind IP] --output FILE" >&2; }
...
write_env "$OUTPUT" \
  ...
  "SEED_API_URL=https://$GENESIS_PUBLIC_HOST" "SEED_NODE_RPC_URL=https://$GENESIS_PUBLIC_HOST/chain-rpc/" \
  "SEED_NODE_P2P_URL=tcp://$GENESIS_PUBLIC_HOST:$GENESIS_P2P_PORT" \
  "RPC_SERVER_URL_1=https://$GENESIS_PUBLIC_HOST/chain-rpc/" "RPC_SERVER_URL_2=https://$PUBLIC_EDGE_HOST/chain-rpc/" \
