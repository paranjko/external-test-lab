...
if [[ -n "$STATE_SYNC_ENV" ]]; then
  source_rpc=''
  [[ -z "$JOIN_PROFILE" ]] || source_rpc="$(jq -r '.spec.state_acquisition.trust_authority | select(.kind == "operator_source") | .rpc_url' "$JOIN_PROFILE")"
  ...
  [[ "${GDC_JOIN_BOOTSTRAP_MODE:-}" =~ ^(state_sync|historical_replay)$ ]] || { echo 'JOIN synchronization environment is incomplete' >&2; exit 1; }
  if [[ "$GDC_JOIN_BOOTSTRAP_MODE" == state_sync ]]; then
    [[ "${GDC_JOIN_TRUST_HEIGHT:-}" =~ ^[1-9][0-9]*$ && "${GDC_JOIN_TRUST_HASH:-}" =~ ^[0-9a-f]{64}$ ]] \
      || { echo 'state-sync environment is incomplete' >&2; exit 1; }
  else
    [[ -r "${GDC_JOIN_UPGRADE_SCHEDULE:-}" && "${GDC_JOIN_UPGRADE_SCHEDULE_SHA256:-}" =~ ^[0-9a-f]{64}$ \
      && "$(sha256sum "$GDC_JOIN_UPGRADE_SCHEDULE" | awk '{print $1}')" == "$GDC_JOIN_UPGRADE_SCHEDULE_SHA256" ]] \
      || { echo 'full-history mode lacks its checked runtime schedule' >&2; exit 1; }
  fi
  if [[ -n "$source_rpc" ]]; then
    [[ "${GDC_JOIN_SOURCE_RPC:-}" == "$source_rpc" ]] || { echo 'source authority differs from Join Profile' >&2; exit 1; }
  fi
  source_rpc="${GDC_JOIN_SOURCE_RPC:-$source_rpc}"
  if [[ -n "$source_rpc" ]]; then
...
join_seeds="$SEEDS"
[[ "$join_pex" != false && -z "${source_rpc:-}" ]] || join_seeds=''
...
write_env "$OUTPUT" \
  ...
  "IS_GENESIS=$IS_GENESIS" "INIT_ONLY=false" "GENESIS_SEEDS=$SEEDS" "SYNC_WITH_SNAPSHOTS=$([[ "${GDC_JOIN_BOOTSTRAP_MODE:-}" == state_sync ]] && echo true || echo false)" \
  "GDC_JOIN_STATESYNC_ENABLE=$([[ "${GDC_JOIN_BOOTSTRAP_MODE:-}" == state_sync ]] && echo true || echo false)" \
