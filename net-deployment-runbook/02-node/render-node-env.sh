...
if [[ -n "$STATE_SYNC_ENV" ]]; then
  ...
  if [[ -n "$source_rpc" ]]; then
    [[ "${GDC_JOIN_RPC_SERVER_1:-}" == "$source_rpc/" && "${GDC_JOIN_RPC_SERVER_2:-}" == "$source_rpc/" && "$GDC_JOIN_SNAPSHOT_PEERS" != *,* ]] \
      || { echo 'operator-source state sync must use only the selected source' >&2; exit 1; }
  fi
  [[ "${GDC_JOIN_RPC_SERVER_1:-}" =~ ^https?://[A-Za-z0-9.-]+(:[1-9][0-9]{0,4})?/chain-rpc/$ && "${GDC_JOIN_RPC_SERVER_2:-}" =~ ^https?://[A-Za-z0-9.-]+(:[1-9][0-9]{0,4})?/chain-rpc/$ \
    && ( "$GDC_JOIN_RPC_SERVER_1" != "$GDC_JOIN_RPC_SERVER_2" || ( -n "$source_rpc" && "$GDC_JOIN_RPC_SERVER_1" == "$source_rpc/" ) ) ]] \
  ...
  [[ "${GDC_JOIN_SNAPSHOT_PEERS:-}" =~ ^[0-9a-f]{40}@tcp://[A-Za-z0-9.-]+:[0-9]{2,5}(,[0-9a-f]{40}@tcp://[A-Za-z0-9.-]+:[0-9]{2,5})*$ \
    && ( -n "$source_rpc" || "$GDC_JOIN_SNAPSHOT_PEERS" == *,* ) ]] \
