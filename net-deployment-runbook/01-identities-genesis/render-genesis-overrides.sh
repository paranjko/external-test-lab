...
usage() { echo "Usage: $0 --gateway-account ADDRESS --output FILE" >&2; }
GATEWAY=''; OUTPUT=''
while (($#)); do
  case "$1" in
...
[[ "$GATEWAY" =~ ^gonka1[0-9a-z]{20,90}$ && -n "$OUTPUT" ]] || { usage; exit 2; }
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$ROOT/scripts/profile.sh"
load_profiles
TEMPLATE="$ROOT/01-identities-genesis/genesis-overrides.template.json"
mkdir -p "$(dirname "$OUTPUT")"
jq --arg model "$MODEL_ID" --arg revision "$MODEL_REVISION" \
