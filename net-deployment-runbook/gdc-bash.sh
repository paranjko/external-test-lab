#!/usr/bin/env bash
# Bash >= 5 implementation reached only through gdc.sh's POSIX bootstrap.
set -Eeuo pipefail

LAUNCHER_SOURCE="${BASH_SOURCE[0]}"
# Establish the adjacent portable module without GNU realpath. It resolves the
# launcher path itself after loading, including a symlinked user invocation.
ROOT="$(CDPATH='' cd -P "$(dirname "$LAUNCHER_SOURCE")" && pwd -P)"
. "$ROOT/scripts/portable.sh"
LAUNCHER_PATH="$(gdc_realpath_existing "$LAUNCHER_SOURCE")" || {
  printf 'path_resolution_failed: gdc launcher path must exist\nmutation=none\n' >&2
  exit 66
}
ROOT="$(CDPATH='' cd -P "$(dirname "$LAUNCHER_PATH")" && pwd -P)"
gdc_require_jq || exit $?
if [[ -n "${GDC_USAGE_COMMAND:-}" ]]; then
  : # The POSIX bootstrap preserves the public invocation spelling.
elif [[ "${LAUNCHER_SOURCE##*/}" == gdc.sh ]]; then
...
GDC_LAUNCHER_TERMINATING=false
GDC_ACTIVE_PHASE_CHILD_PID_FILE=''

on_launcher_signal() {
  local signal="$1"
  # EXIT must retain the owner-bound lifecycle lock while a mutating phase is
  # being interrupted.  It will stop and reap the phase child before leaving
  # the lock as recovery evidence for the next invocation.
  GDC_LAUNCHER_TERMINATING=true
  GDC_LAUNCHER_SIGNAL="$signal"
  exit 128
}
...
record_join_terminal_result() {
  ...
  if [[ -r "${GDC_JOIN_PROFILE:-}" ]]; then
    profile_sha="\"$(gdc_sha256 "$GDC_JOIN_PROFILE")\""
  fi
...
on_launcher_exit() {
  ...
  trap - EXIT
  set +e
  if [[ "$GDC_LAUNCHER_TERMINATING" == true ]]; then
    if [[ -n "$GDC_ACTIVE_PHASE_CHILD_PID_FILE" && -r "$GDC_ACTIVE_PHASE_CHILD_PID_FILE" ]]; then
      local child_pid
      child_pid="$(<"$GDC_ACTIVE_PHASE_CHILD_PID_FILE")"
      [[ "$child_pid" =~ ^[0-9]+$ ]] && kill -TERM "$child_pid" 2>/dev/null || true
      [[ "$child_pid" =~ ^[0-9]+$ ]] && wait "$child_pid" 2>/dev/null || true
    fi
    [[ -z "${GDC_ACTIVE_PHASE_PIPELINE_PID:-}" ]] || {
      kill -TERM "$GDC_ACTIVE_PHASE_PIPELINE_PID" 2>/dev/null || true
      wait "$GDC_ACTIVE_PHASE_PIPELINE_PID" 2>/dev/null || true
    }
    printf 'INTERRUPTED phase=%s signal=%s lifecycle lock retained for recovery\n' \
      "${GDC_ACTIVE_PHASE:-unavailable}" "${GDC_LAUNCHER_SIGNAL:-unknown}" >&2
  else
    gdc_lock_release
  fi
...
acquire_operator_lock() {
  ...
  gdc_lock_acquire "$STATE" "$GDC_LAUNCHER_INVOCATION_ID" "${GDC_INVOCATION_COMMAND:-gdc}" || exit $?
...
run_phase() {
  local phase="$1"
  shift
  local state run_id_file run_id run_dir log rc diagnostic_envelope phase_status_file phase_pipeline_pid
  state="$STATE"
  acquire_operator_lock
  ...
  set +e
  phase_status_file="$run_dir/.phase-status.$$"
  GDC_ACTIVE_PHASE_CHILD_PID_FILE="$run_dir/.phase-child.$$"
  export GDC_ACTIVE_PHASE_CHILD_PID_FILE
  {
    printf '%s\n' "$BASHPID" >"$GDC_ACTIVE_PHASE_CHILD_PID_FILE"
    ...
    printf '%s\n' "$rc" >"$phase_status_file"
    exit "$rc"
  } 2>&1 | tee -a "$log" &
  phase_pipeline_pid=$!
  GDC_ACTIVE_PHASE_PIPELINE_PID="$phase_pipeline_pid"
  export GDC_ACTIVE_PHASE_PIPELINE_PID
  wait "$phase_pipeline_pid" || true
  if [[ -r "$phase_status_file" ]]; then
    rc="$(<"$phase_status_file")"
  else
    rc=143
  fi
  rm -f "$phase_status_file" "$GDC_ACTIVE_PHASE_CHILD_PID_FILE"
  GDC_ACTIVE_PHASE_CHILD_PID_FILE=''
  unset GDC_ACTIVE_PHASE_PIPELINE_PID
  if (( rc != 0 )); then
    diagnostic_envelope="$(gdc_find_regular_beneath_depth2 "$run_dir" diagnostic-envelope.v1.json | tail -n1 || true)"
  ...
  if [[ "$phase" == join-* && -n "${GDC_JOIN_RESULT_OUTPUT:-}" ]]; then
    ...
    if [[ "${GDC_JOIN_PRESERVE_PRIOR_RUN:-false}" == true || -n "$(gdc_find_regular_beneath_depth2 "$run_dir" preserve-prior-run | tail -n1 || true)" ]]; then
  ...
  set -e
  # A multi-Host command changes STATE between phases.  Release this phase's
  # owner-bound directory before the next alias acquires its own lock.
  gdc_lock_release
  unset GDC_OPERATOR_LOCK_STATE
  return "$rc"
}
...
See the role guides for required input, then run:
  ...
  ./gdc.sh host lock inspect <SSH_ALIAS>
...
case "$COMMAND" in
  ...
  host)
    ...
    case "$subcommand" in
      ...
      lock)
        lock_action="${1:-}"; shift || true
        [[ "$lock_action" == inspect ]] || { usage; exit 2; }
        COMMAND='host-lock-inspect'
        ;;
      upgrade)
...
case "$COMMAND" in
  ...
  host-lock-inspect)
    lock_alias="${1:-}"
    [[ $# -eq 1 && "$lock_alias" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || { echo 'host lock inspect requires exactly one SSH alias' >&2; exit 2; }
    use_node_data_home "$lock_alias"
    if gdc_lock_inspect "$STATE"; then
      :
    else
      rc=$?
      if [[ "$rc" -eq 1 ]]; then
        printf 'No lifecycle lock is present for %s.\n' "$lock_alias"
        exit 0
      fi
      printf 'lock_io_error: cannot inspect lifecycle lock\nmutation=none\n' >&2
      exit "$rc"
    fi
    ;;
  ...
  join)
    ...
    while [[ $# -gt 0 ]]; do
      case "$1" in
        ...
        --old-signer-fence)
          ...
          join_old_signer_fence="$(gdc_realpath_existing "$join_old_signer_fence")"
          shift
          ;;
        --chain-id)
          join_chain_id="${2:-}"
        ...
        --bootstrap-file)
          ...
          join_bootstrap_file="$(gdc_realpath_existing "$join_bootstrap_file")"
          shift
          ;;
        --p2p-port)
          join_p2p_port="${2:-}"
        ...
        --restore)
          ...
          join_restore_archive="$(gdc_realpath_existing "$join_restore_archive")"
          shift
          ;;
    ...
    if [[ -n "$join_previous_run_id" && "$join_previous_run_id" != "$GDC_RUN_ID" ]]; then
      ...
      case "$join_reentry_class" in
        no_prior_run)
          ;;
        completed_matched)
          head_name="$(gdc_latest_join_receipt_name "$previous_join_run/receipts")" || {
            echo 'host join completed-state evidence has no safely ordered receipt name' >&2
            exit 1
          }
    ...
    GDC_JOIN_LINEAGE_RECEIPT_SHA256="$(gdc_sha256 "$GDC_JOIN_LINEAGE_RECEIPT")"
