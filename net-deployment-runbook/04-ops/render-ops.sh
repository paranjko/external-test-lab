...
# Builder and Compose must select the same protocol-specific local image.
# The candidate's native protocol keeps the checksum-bound archive tag;
# alternate governed protocols use distinct local tags.
LOCAL_GATEWAY_IMAGE="$(local_gateway_image_for_protocol "$GATEWAY_VERSION")"
