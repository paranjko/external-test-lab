...
case "$url" in
  ...
  */v1/versions)
    ...
    [[ "${MODE:-good}" != incomplete || "$node" != 0 ]] || { printf '{"node_version":{"version":"0.2.14"}}\n'; exit 0; }
    [[ "${MODE:-good}" != declared_api_unavailable || "$node" != 0 ]] || exit 7
    api_node_version="$version"; api_node_commit="$commit"
    [[ "${MODE:-good}" != api_mismatch || "$node" != 0 ]] || { api_node_version=0.2.14; api_node_commit=2bfd85c958732992c7a9c5be1d796affe29f3ab4; }
    dapi_version=0.2.15-post3; dapi_commit=5dbb53ddf3ddc42655fc04dc39d96003169bdbb0
    [[ "${MODE:-good}" != declared_api_conflict || "$node" != 1 ]] || { dapi_version=0.2.16; dapi_commit=18506d42c510e0cafe6acd748bcd8d83036cba40; }
    [[ "${MODE:-good}" != undeclared_dapi_drift || "$node" != 2 ]] || { dapi_version=0.2.16; dapi_commit=18506d42c510e0cafe6acd748bcd8d83036cba40; }
    printf '{"node_version":{"version":"%s","commit":"%s"},"api_version":{"version":"%s","commit":"%s"}}\n' "$api_node_version" "$api_node_commit" "$dapi_version" "$dapi_commit"
    ;;
...
run_case() {
  local mode="$1" bootstrap="${2:-$tmp/bootstrap.json}" output=''
  output="$tmp/$mode.env"
  MODE="$mode" PATH="$tmp/bin:$PATH" "$OBSERVE" --bootstrap-file "$bootstrap" --output "$output"
}
run_case good
...
MODE=undeclared_dapi_drift CURL_SPY="$tmp/undeclared-api-spy" PATH="$tmp/bin:$PATH" "$OBSERVE" --bootstrap-file "$tmp/bootstrap.json" --output "$tmp/undeclared_dapi_drift.env"
! grep -Fq 'node2.example.test/v1/versions' "$tmp/undeclared-api-spy"
run_case reordered
# shellcheck source=/dev/null
...
jq '.seeds[1].api = "https://node1.example.test"' "$tmp/bootstrap.json" >"$tmp/multiple-apis.json"
if run_case declared_api_conflict "$tmp/multiple-apis.json" >"$tmp/declared_api_conflict.out" 2>"$tmp/declared_api_conflict.err"; then
  echo 'conflicting declared APIs were accepted' >&2
  exit 1
fi
...
grep -Fq 'software_ambiguous:' "$tmp/declared_api_conflict.err"
grep -Fq 'software_declared_api_unavailable:' "$tmp/declared_api_unavailable.err"
