...
set -Eeuo pipefail
target="${!#}"
if [[ "$target" == *host-stack-compose.yml ]]; then
  printf '%s  %s\n' d4b17a18013160236b79aac880a9f5b17705312f45c85ea3d37cc978c8da3f94 "$target"
else
  /usr/bin/shasum "$@"
fi
EOF
...
mkdir -p "$tmp/fixtures – ü"
cp "$tmp/bootstrap.json" "$tmp/fixtures – ü/bootstrap file.json"
printf 'retained validator archive\n' >"$tmp/fixtures – ü/validator backup.tar"
: >"$tmp/remote-effects.log"
PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --plan --bootstrap-file "$tmp/fixtures – ü/bootstrap file.json" --restore "$tmp/fixtures – ü/validator backup.tar" \
    --skip-qualification --public-host validator-a.example.test validator-a >"$tmp/out" 2>"$tmp/err"

[[ ! -s "$tmp/remote-effects.log" ]]
! find "$tmp/operator state – ü" -type d -name genesis -print -quit | grep -q .
profile="$(find "$tmp/operator state – ü" -type f -path '*/join-validator-a/join-profile.v1.json' -print -quit)"
observation="$(find "$tmp/operator state – ü" -type f -path '*/join-validator-a/network-observation.v1.json' -print -quit)"
receipt="$(find "$tmp/operator state – ü" -type f -path '*/join-validator-a/preflight-receipt.env' -print -quit)"
result="$(find "$tmp/operator state – ü" -type f -path '*/join-validator-a/join-result.v1.json' -print -quit)"
[[ -n "$profile" && -n "$observation" && -n "$receipt" && -n "$result" ]]
[[ "$(gdc_file_mode "$profile")" == 600 && "$(gdc_file_mode "$result")" == 600 ]]
[[ ! -e "$tmp/operator state – ü/validator-a/state/active-run-id" ]] || { echo '--plan unexpectedly became the active JOIN run' >&2; exit 1; }
...
profile_output=$(gdc_realpath_existing "$profile")
result_output=$(gdc_realpath_existing "$result")
grep -Fq "profile=$profile_output" "$tmp/out"
grep -Fq "result=$result_output" "$tmp/out"
...
completed_run="$tmp/operator state – ü/validator-a/runs/$completed_id/join-validator-a"
...
completed_profile_sha256="$(gdc_sha256 "$completed_run/join-profile.v1.json")"
...
printf '%s\n' "$completed_id" >"$tmp/operator state – ü/validator-a/state/active-run-id"
cat >"$tmp/bin/ssh" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
...
if PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --plan --resume "$completed_id" --public-host validator-a.example.test validator-a >"$tmp/complete-resume-invalid.out" 2>"$tmp/complete-resume-invalid.err"; then
...
PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --plan --resume "$completed_id" --public-host validator-a.example.test validator-a >"$tmp/complete-resume-valid.out" 2>"$tmp/complete-resume-valid.err"
...
if ! PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --bootstrap-file "$tmp/fixtures – ü/bootstrap file.json" --restore "$tmp/fixtures – ü/validator backup.tar" \
...
PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --plan --resume "$run_id" --public-host validator-a.example.test validator-a >"$tmp/resume.out" 2>"$tmp/resume.err"
[[ ! -s "$tmp/remote-effects.log" ]]
...
: >"$tmp/remote-effects.log"
if PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --resume "$run_id" --public-host validator-a.example.test validator-a >"$tmp/unsupported-resume.out" 2>"$tmp/unsupported-resume.err"; then
...
if PATH="$tmp/bin:$PATH" GDC_PLAN_REMOTE_EFFECT_LOG="$tmp/remote-effects.log" GDC_HOME="$tmp/operator state – ü" \
  "$ROOT/gdc-bash.sh" host join --plan --resume "$run_id" --public-host validator-a.example.test validator-a >"$tmp/tampered-resume.out" 2>"$tmp/tampered-resume.err"; then
