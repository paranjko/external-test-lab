...
[[ -s "$SECRETS/operator.keyring" ]] || die 'controller operator key is required for activation'
...
[[ "$ADDRESS" == "$EXPECTED" ]] || die 'activation request cold address does not match controller-created account'
