...
grep -Fq 'host join --public-host <IP_or_DOMAIN> <ssh-alias>' "$DOC"
grep -Eq '^cleanroom-shell:$' "$ROOT/Makefile"
grep -Fq 'cleanroom cleanroom-reset cleanroom-shell: export SSH_AUTH_SOCK := $(CLEANROOM_SSH_AUTH_SOCK)' "$ROOT/Makefile"
grep -Fq 'usermod --append --groups root' "$ROOT/.devcontainer/cleanroom/Dockerfile"
