...
grep -Fq 'if [[ -n "$GATEWAY_NODE" ]]; then' "$ROOT/04-ops/edge-node/render-env.sh"
grep -Fq 'gateway_admission_status_url="https://${API_HOST}/v1/status"' "$ROOT/04-ops/edge-node/render-env.sh"
grep -Fq 'gateway_admission_status_url="https://${gateway_public_host}/ops-gateway-admission-state"' "$ROOT/04-ops/edge-node/render-env.sh"
