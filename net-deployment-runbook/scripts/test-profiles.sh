...
grep -Fq 'time() - gdc_gateway_readiness_observed_timestamp_seconds > 30' "$ROOT/04-ops/prometheus/alerts.yml"
...
grep -Fq 'GDC_GATEWAY_ADMISSION_STATUS_MAX_WAIT_SECONDS", 5' "$ROOT/04-ops/edge-node/gateway-admission-proxy.py"
grep -Fq 'GDC_GATEWAY_ADMISSION_STATUS_MAX_WAIT_SECONDS=${GDC_GATEWAY_ADMISSION_STATUS_MAX_WAIT_SECONDS:-5}' "$ROOT/04-ops/edge-node/render-env.sh"
...
grep -Fq 'handle /status/gateway/v1/admission-status {' "$ROOT/04-ops/edge-node/PublicCaddyfile"
grep -Fq 'rewrite * /v1/admission-status' "$ROOT/04-ops/edge-node/PublicCaddyfile"
grep -Fq 'reverse_proxy 127.0.0.1:18083' "$ROOT/04-ops/edge-node/PublicCaddyfile"
...
grep -Fq "s/^FAUCET_GATEWAY_RESERVE_MAX_NGONKA=//p' /srv/dai/ops/gateway-reserve-signer.env" "$ROOT/scripts/phase-ops.sh"
grep -Fq "GDC_GATEWAY_MAX_REFILL_NGONKA does not match the deployed gateway reserve signer maximum" "$ROOT/scripts/phase-ops.sh"
grep -Fq 'export GDC_GATEWAY_MAX_REFILL_NGONKA="$gateway_max_refill"' "$ROOT/scripts/phase-ops.sh"
...
grep -Fq '(time() - max(gdc_telegram_bot_last_success_timestamp_seconds))' "$ROOT/04-ops/grafana/generate-dashboards.sh"
grep -Fq 'rate(devshard_gateway_participant_total_attempt_seconds_count[6h])' "$ROOT/04-ops/grafana/generate-dashboards.sh"
grep -Fq 'Traffic ready' "$ROOT/04-ops/grafana/generate-dashboards.sh"
grep -Fq 'gdc_gateway_readiness_state' "$ROOT/04-ops/grafana/generate-dashboards.sh"
grep -Fq 'devshard_gateway_escrow_blocked_participants' "$ROOT/04-ops/grafana/generate-dashboards.sh"
grep -Fq 'required-panel-expressions.txt' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'Request, executor and escrow series are intentionally' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'required panel expression is no longer on a served dashboard' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'public Grafana serves a panel that measures nothing and says nothing' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'panel-inventory.tsv' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq '$(wc -l <"$RUN/required-panel-expressions.txt")' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'window-size=1440,3600' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'window-size=1440,6400' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'That tolerance is not' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'classify_panel_results' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'QUERY_FAILED' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'anonymous public datasource proxy refused a pinned expression' "$ROOT/scripts/verify-public-grafana.sh"
grep -Fq 'judgement_deadline=$((SECONDS + 180))' "$ROOT/scripts/verify-public-grafana.sh"
...
grep -Fq 'rm -rf -- /srv/dai "/tmp/gdc-deploy-"*-' "$ROOT/scripts/phase-node.sh"
...
grep -Fq 'query=last_over_time(gdc_nvidia_memory_total_bytes%5B24h%5D)' "$ROOT/04-ops/render-ops.sh"
...
grep -Fq 'gateway_public_host="$PUBLIC_EDGE_HOST"' "$ROOT/04-ops/edge-node/render-env.sh"
grep -Fq '[[ -z "${GATEWAY_NODE:-}" ]] || gateway_public_host=' "$ROOT/04-ops/edge-node/render-env.sh"
grep -Fq 'telegram_bot_public_host="$PUBLIC_EDGE_HOST"' "$ROOT/04-ops/edge-node/render-env.sh"
grep -Fq 'https://${gateway_public_host}/ops-gateway-admission-state' "$ROOT/04-ops/edge-node/render-env.sh"
...
grep -Fq '<summary><span class="join-requirements-toggle">Minimum Host requirements</span> <span class="join-requirements-guide">For more details, read <a href="https://github.com/paranjko/external-test-lab/blob/main/net-deployment-runbook/ROLE-JOIN.md#join-add-a-host"' "$ROOT/04-ops/site/index.html"
! grep -Fq '<p>For more details, read' "$ROOT/04-ops/site/index.html"
node "$ROOT/scripts/verify-devnet-hardware-profile.mjs" \
  "$ROOT/profiles/devnet-hadware.json" "$ROOT/04-ops/site/index.html"
grep -Fq 'https://node0.gonka-dev.net/chain-api/productscience/inference/inference/models_all' "$ROOT/04-ops/site/index.html"
grep -Fq 'qualification loads the model, checks <code>/v1/models</code>, completes an inference request, and records VRAM evidence' "$ROOT/04-ops/site/index.html"
...
grep -Fq "v${site_commit:0:7}" "$rendered_site_index"
...
grep -Fq 'install -m 0755 "$(dirname "$0")/api-entrypoint.sh" "$STAGE/api-entrypoint.sh"' "$ROOT/02-node/install-node.sh"
grep -Fq '"$ROOT/02-node/api-entrypoint.sh"' "$ROOT/scripts/phase-explorer.sh"
grep -Fq "'\$REMOTE/api-entrypoint.sh'" "$ROOT/scripts/phase-explorer.sh"
