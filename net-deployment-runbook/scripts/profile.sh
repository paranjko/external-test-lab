...
load_profiles() {
  ...
  unset DEVSHARD_GATEWAY_IMAGE_ARCHIVE_SHA256
  ...
  if [[ "${LAB_CANDIDATE:-false}" == true ]]; then
    DEVSHARD_SOURCE_REF="$CANDIDATE_DEVSHARD_SOURCE_REF"
    DEVSHARD_COMMIT="$CANDIDATE_DEVSHARD_COMMIT"
    DEVSHARD_PROTOCOL_VERSION="$CANDIDATE_DEVSHARD_PROTOCOL_VERSION"
    DEVSHARD_SUPPORTED_PROTOCOLS="$CANDIDATE_DEVSHARD_SUPPORTED_PROTOCOLS"
    LOCAL_GATEWAY_IMAGE="$CANDIDATE_LOCAL_GATEWAY_IMAGE"
    POSTGRES_IMAGE="$CANDIDATE_POSTGRES_IMAGE"
