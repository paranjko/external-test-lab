...
grep -Fq 'path: request-source' "$PUBLISH_WORKFLOW"
grep -Fq 'ref: ${{ github.workflow_sha }}' "$PUBLISH_WORKFLOW"
grep -Fq 'path: automation' "$PUBLISH_WORKFLOW"
...
grep -Fq 'candidate definition sidecar mismatch' "$PUBLISH_WORKFLOW"
...
grep -Fq 'release_tag: ${{ steps.definition.outputs.release_tag }}' "$PUBLISH_WORKFLOW"
grep -Fq 'release_url_segment: ${{ steps.definition.outputs.release_url_segment }}' "$PUBLISH_WORKFLOW"
...
grep -Fq 'name: Render candidate release notes' "$PUBLISH_WORKFLOW"
grep -Fq 'render-candidate-release-notes.py' "$PUBLISH_WORKFLOW"
grep -Fq -- '--notes-file release-notes.md' "$PUBLISH_WORKFLOW"
grep -Fq -- '--latest' "$PUBLISH_WORKFLOW"
! grep -Fq -- '--prerelease' "$PUBLISH_WORKFLOW"
grep -Fq "'{name:\$name,body:\$body,prerelease:false,make_latest:true}'" "$PUBLISH_WORKFLOW"
grep -Fq 'gh api --method PATCH "repos/$GITHUB_REPOSITORY/releases/$release_id" --input release-edit.json' "$PUBLISH_WORKFLOW"
! grep -Fq -- '--notes "Independent laboratory build.' "$PUBLISH_WORKFLOW"
...
grep -Fq 'request-source/net-deployment-runbook/profiles/candidates/$PROFILE.definition.json' <<<"$prepare_job"
grep -Fq 'cp "$definition" "$trusted_candidates/$PROFILE.definition.json"' <<<"$prepare_job"
grep -Fq 'cp "$definition_sidecar" "$trusted_candidates/$PROFILE.definition.sha256"' <<<"$prepare_job"
grep -Fq 'python3 automation/net-deployment-runbook/scripts/release-candidate.py' <<<"$prepare_job"
...
grep -Fq 'RELEASE_NAME: ${{ matrix.release_name }}' <<<"$publish_binaries_job"
grep -Fq 'published-binary/$RELEASE_NAME' <<<"$publish_binaries_job"
grep -Fq 'releases/download/$RELEASE_URL_SEGMENT/$RELEASE_NAME' <<<"$publish_binaries_job"
grep -Fq 'name: candidate-binary-release-${{ matrix.id }}' <<<"$publish_binaries_job"
grep -Fq 'pattern: candidate-binary-release-*' <<<"$manifest_job"
! grep -Fq 'pattern: candidate-binary-*' <<<"$manifest_job"
grep -Fq 'tag="$RELEASE_TAG"' <<<"$manifest_job"
! grep -Fq 'releases/download/lab-candidate%2F$PROFILE' "$PUBLISH_WORKFLOW"
...
grep -Fq './gdc.sh release candidate prepare --source-ref ak/height-sync-protocol-dapi --layer core' "$RUNBOOK/gdc-bash.sh"
...
grep -Fq '"$ROOT/scripts/release-candidate.py" "$@"' "$RUNBOOK/gdc-bash.sh"
! grep -Fq 'exec "$ROOT/scripts/release-candidate.py" "$@"' "$RUNBOOK/gdc-bash.sh"
