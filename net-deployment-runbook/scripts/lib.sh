...
# A completed generated JOIN is governed by its retained immutable profile,
# not by a later launcher-default release. Lifecycle callers know the Host
# alias before loading the project, so this is a local, network-free lookup.
load_retained_join_profile_for_node() {
  local node="$1" run manifest profile expected_sha actual_sha
  [[ "$node" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]] || die 'invalid Host alias for retained JOIN profile'
  [[ -s "$STATE/active-run-id" ]] || return 0
...
load_project() {
  ...
  load_topology
  mapfile -t genesis_addresses < <(python3 "$ROOT/scripts/resolve-ipv4.py" "$GENESIS_PUBLIC_HOST" | awk '{print $1}' | sort -u)
  ...
  mapfile -t edge_addresses < <(python3 "$ROOT/scripts/resolve-ipv4.py" "$PUBLIC_EDGE_HOST" | awk '{print $1}' | sort -u)
