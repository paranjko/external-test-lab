...
[[ "$initial_epoch" == 40 && "$deadline_epoch" == 43 ]]
jq -e --arg run_id "$run_id" '.run_id == $run_id and .initial_epoch == 40 and .deadline_epoch == 43 and .strongest_observed.epoch == 41' "$run/acceptance-state.json" >/dev/null
printf 'PASS interrupted epoch-40 acceptance resumes at epoch 67 with original deadline 43\n'
