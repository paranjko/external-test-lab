...
grep -Fq 'REMOTE_JOIN_PROFILE' "$RECOVERY"
grep -Fq 'verify-canonical-join-state.sh' "$RECOVERY"
grep -Fq 'EXPECTED_PROFILE_CORE_COMMIT' "$RECOVERY"
grep -Fq 'EXPECTED_PROFILE_DAPI_COMMIT' "$RECOVERY"
grep -Fq 'REMOTE_RELEASE' "$RECOVERY"
grep -Fq 'The joined marker is the only commit point' "$PUBLISHER"
grep -Fq 'gdc_lock_acquire "$lock_file" 0' "$PUBLISHER"
