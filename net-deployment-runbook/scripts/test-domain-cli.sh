...
grep -Fq 'Poll for a fresh post-recovery public traffic-readiness receipt' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq 'wait-public-traffic-readiness.sh' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq 'verification_deadline_ms=' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq 'remaining_verification_seconds()' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq 'inference_timeout_seconds="$(remaining_verification_seconds authenticated_inference)"' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq 'readiness_timeout_seconds="$(remaining_verification_seconds public_readiness)"' "$ROOT/scripts/phase-gateway-observe.sh"
grep -Fq '"$max_age_seconds" "$readiness_timeout_seconds" "$readiness_poll_seconds"' "$ROOT/scripts/phase-gateway-observe.sh"
! grep -Fq '"$max_age_seconds" "${sla%s}" "$readiness_poll_seconds"' "$ROOT/scripts/phase-gateway-observe.sh"
