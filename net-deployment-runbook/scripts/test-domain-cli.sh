...
for contract in \
  ...
  grep -Fq "$contract" "$ROOT/gdc-bash.sh"
done

grep -Fq 'case "$COMMAND" in' "$ROOT/gdc-bash.sh"
grep -Fq 'phase-gateway-canary.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'format_safe_invocation' "$ROOT/gdc-bash.sh"
grep -Fq 'INVOCATION command=%s' "$ROOT/gdc-bash.sh"
grep -Fq 'invocation_command=%q' "$ROOT/scripts/lib.sh"
grep -Fq 'prepare-join-role-config.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_JOIN_SKIP_QUALIFICATION="$skip_qualification"' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_JOIN_VERIFICATION="$verification"' "$ROOT/gdc-bash.sh"
grep -Fq "skip_qualification=false verification=false plan_only=false" "$ROOT/gdc-bash.sh"
grep -Fq 'host join does not accept an externally authored signer-fence receipt' "$ROOT/gdc-bash.sh"
grep -Fq 'verify-join-resume-inputs.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_RESTORE_VALIDATOR_BACKUP_ARCHIVE="$join_restore_archive"' "$ROOT/gdc-bash.sh"
grep -Fq 'host join does not accept --release' "$ROOT/gdc-bash.sh"
grep -Fq 'observe-network-state.sh' "$ROOT/gdc-bash.sh"
! grep -Fq 'observe-network-composition.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'export GDC_RELEASE_PROFILE' "$ROOT/gdc-bash.sh"
join_release_tmp="$(mktemp -d)"
trap 'rm -rf "$tmp" "$join_release_tmp"' EXIT
if GDC_HOME="$join_release_tmp" "$ROOT/gdc-bash.sh" --release v2026.07.23 host join node1 >"$join_release_tmp/out" 2>"$join_release_tmp/err"; then
...
resume_tmp="$(mktemp -d)"
if GDC_HOME="$resume_tmp" "$ROOT/gdc-bash.sh" host join --resume retained-run --public-host node9.example.test node9 >"$resume_tmp/out" 2>"$resume_tmp/err"; then
...
for selector in '--release=v2026.07.23' '--release v2026.07.23'; do
  ...
  if GDC_HOME="$join_release_tmp" "$ROOT/gdc-bash.sh" host join "${selector_args[@]}" node1 >"$join_release_tmp/local.out" 2>"$join_release_tmp/local.err"; then
...
grep -Fq 'GDC_JOIN_RECOVERY_NEW_RUN:-false' "$ROOT/gdc-bash.sh"
join_run_id_line="$(grep -n 'elif \[\[ -n "\${GDC_RUN_ID:-}" \]\]; then' "$ROOT/gdc-bash.sh" | head -1 | cut -d: -f1)"
recovery_run_line="$(grep -n 'GDC_JOIN_RECOVERY_NEW_RUN:-false' "$ROOT/gdc-bash.sh" | head -1 | cut -d: -f1)"
...
grep -Fq 'host backup requires retained operator state' "$ROOT/gdc-bash.sh"
grep -Fq 'phase-host-backup.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'join_config_args+=(--public-host "$join_public_host")' "$ROOT/gdc-bash.sh"
grep -Fq 'detect-public-host.sh" "$SSH_ALIAS"' "$ROOT/scripts/prepare-join-role-config.sh"
grep -Fq 'join_config_args+=(--gpu-ssh-alias "$join_gpu_alias")' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_NODE_ML_HOSTS=' "$ROOT/scripts/prepare-join-role-config.sh"
grep -Fq 'shell_quote "${GPU_SSH_ALIAS:+$SSH_ALIAS=$GPU_SSH_ALIAS}"' "$ROOT/scripts/prepare-join-role-config.sh"
...
grep -Fq 'join_chain_id=gonka-devnet-community' "$ROOT/gdc-bash.sh"
grep -Fq 'join_bootstrap_url="https://gonka-dev.net/${join_chain_id}/bootstrap.json"' "$ROOT/gdc-bash.sh"
grep -Fq -- '--chain-id=*)' "$ROOT/gdc-bash.sh"
grep -Fq 'network bootstrap verify FILE' "$ROOT/gdc-bash.sh"
...
grep -Fq '^[a-z0-9][a-z0-9_-]*$' "$ROOT/gdc-bash.sh"
if GDC_HOME="$tmp" "$ROOT/gdc-bash.sh" host join --chain-id ../unsafe --public-host node2.example.net gdc-node2 >"$tmp/unsafe-chain.stdout" 2>"$tmp/unsafe-chain.stderr"; then
...
if GDC_HOME="$tmp" "$ROOT/gdc-bash.sh" host join --chain-id=../unsafe --public-host node2.example.net gdc-node2 >"$tmp/unsafe-chain-equals.stdout" 2>"$tmp/unsafe-chain-equals.stderr"; then
...
for requested_chain in gonka-devnet-community gonka-testnet; do
  ...
  if PATH="$chain_probe/bin:$PATH" CHAIN_PROBE_LOG="$chain_probe/$requested_chain.log" CHAIN_PROBE_BOOTSTRAP="$chain_probe/bootstrap.json" GDC_HOME="$chain_probe/$requested_chain" "$ROOT/gdc-bash.sh" "${args[@]}" >"$chain_probe/$requested_chain.out" 2>"$chain_probe/$requested_chain.err"; then
...
grep -Fq '[[ "${GDC_JOIN_ROLE_INPUT:-false}" != true ]] || exit 1' "$ROOT/gdc-bash.sh"
! grep -Fq 'join-bootstrap-dispatched.manifest.sha256' "$ROOT/gdc-bash.sh"
! grep -Fq 'join-bootstrap-dispatched.manifest.sha256' "$ROOT/scripts/phase-join.sh"
grep -Fq 'acquire_operator_lock' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_OPERATOR_LOCK_STATE' "$ROOT/gdc-bash.sh"
! grep -Fq 'JOIN dispatch binding disagrees with the selected role input' "$ROOT/gdc-bash.sh"
grep -Fq 'join_config_args+=(--bootstrap-file "$join_bootstrap_file")' "$ROOT/gdc-bash.sh"
grep -Fq '"$ROOT/scripts/ensure-inferenced-cli.sh"' "$ROOT/gdc-bash.sh"
grep -Fq 'join_genesis="$GDC_HOME/genesis"' "$ROOT/gdc-bash.sh"
grep -Fq 'join_secrets="$STATE/secrets"' "$ROOT/gdc-bash.sh"
grep -Fq ' --genesis-dir "$join_genesis" --state-dir "$STATE" --secrets-dir "$join_secrets"' "$ROOT/gdc-bash.sh"
...
if GDC_HOME="$tmp" "$ROOT/gdc-bash.sh" host join --public-host node2.example.net Validator.West >"$tmp/invalid-alias.stdout" 2>"$tmp/invalid-alias.stderr"; then
...
grep -Fq 'COMMAND=node; set -- "$subcommand" "$@"' "$ROOT/gdc-bash.sh"
grep -Fq 'ERROR gdc command failed phase=%s exit=%s run_log=%s command=%s' "$ROOT/gdc-bash.sh"
grep -Fq 'run_phase "gateway-$gateway_action-$GDC_GATEWAY_VERSION"' "$ROOT/gdc-bash.sh"
grep -Fq 'phase-gateway-migration.sh" prepare "$1"' "$ROOT/gdc-bash.sh"
...
! grep -Fq -- '--fresh-state' "$ROOT/gdc-bash.sh"
grep -Fq 'phase-gateway-observe.sh' "$ROOT/gdc-bash.sh"
...
grep -Fq 'phase-bridge-observer.sh' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_GOVERNANCE_SUBMIT=true run_phase' "$ROOT/gdc-bash.sh"
grep -Fq 'configure_devshard_governance_protocols "$2"' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_GOVERNANCE_PROPOSAL_ID="$proposal_id" run_phase' "$ROOT/gdc-bash.sh"
grep -Fq 'composition export-env "$COMPOSITION"' "$ROOT/gdc-bash.sh"
...
if GDC_HOME="$tmp/governance-home" "$ROOT/gdc-bash.sh" \
