...
state_rank() {
  case "$1" in
    ...
    VALIDATOR_KEY_BOUND) echo 155 ;; CANONICAL_RUNNING) echo 160 ;; APPLICATION_ACTIVE) echo 170 ;; CANONICAL_VERIFIED) echo 180 ;;
    RECOVERY_ARCHIVE_READY) echo 185 ;;
    MEMBERSHIP_RECONCILED) echo 190 ;; PERMISSIONS_RECONCILED) echo 200 ;; SIGNER_FENCE_VERIFIED) echo 210 ;;
    SIGNER_ACTIVATING) echo 220 ;; SIGNER_ARMED_PENDING_ELIGIBILITY) echo 230 ;;
    SIGNER_ACTIVE_VERIFIED) echo 240 ;; ACTIVE_CONFIRMED) echo 250 ;;
    RECOVERY_ARCHIVE_VERIFIED) echo 260 ;; COMPLETE) echo 270 ;; REFUSED|FAILED) echo 280 ;;
    *) return 1 ;;
  esac
}
...
jq -e '
  ...
  (.state | IN("RUN_CREATED","BOOTSTRAP_VERIFIED","NETWORK_OBSERVED","JOIN_PROFILE_READY","TARGET_CLASSIFIED","HOST_BASE_PREPARED","IDENTITY_READY","CANDIDATE_RENDERED","CANARY_RUNNING","CANARY_CAUGHT_UP","CANARY_VERIFIED","CANARY_STOPPED","PROMOTION_PREPARED","PROMOTING","PROMOTED","VALIDATOR_KEY_BOUND","CANONICAL_RUNNING","CANONICAL_VERIFIED","APPLICATION_ACTIVE","RECOVERY_ARCHIVE_READY","MEMBERSHIP_RECONCILED","PERMISSIONS_RECONCILED","SIGNER_FENCE_VERIFIED","SIGNER_ACTIVATING","SIGNER_ARMED_PENDING_ELIGIBILITY","SIGNER_ACTIVE_VERIFIED","ACTIVE_CONFIRMED","RECOVERY_ARCHIVE_VERIFIED","COMPLETE","REFUSED","FAILED")) and
