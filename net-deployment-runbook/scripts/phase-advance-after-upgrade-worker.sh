...
load_project
[[ "$GDC_RELEASE_PROFILE" == testnet-0.2.15 ]] || die 'follow-up worker target must be testnet-0.2.15'
...
systemd-run --user --unit="$unit" --working-directory="$ROOT" \
  ...
  /usr/bin/env "GDC_ADVANCE_AFTER_UPGRADE_WAIT_SECONDS=${GDC_ADVANCE_AFTER_UPGRADE_WAIT_SECONDS:-28800}" \
  ./gdc.sh --release testnet-0.2.15 advance-after-upgrade "$proposal_id"

for _ in $(seq 1 20); do
  if systemctl --user is-active --quiet "$unit.service"; then
    cat >"$ROOT/artifacts/runs/${GDC_RUN_ID}-advance-after-upgrade-worker.md" <<EOF
