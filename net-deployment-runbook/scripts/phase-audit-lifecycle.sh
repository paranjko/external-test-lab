...
done < <(find "$ROOT/artifacts/reset-manifests" -mindepth 1 -maxdepth 1 -type d -print 2>/dev/null | LC_ALL=C sort)
...
for item in \
  ...
  'Sepolia bridge|*-bridge-sepolia/*|# Sepolia bridge: PASS' \
  ...
  if [[ "$label" == 'P0 reduced topology' || "$label" == 'gateway continuity' ]]; then
    if found="$(require_pass_bundle "$pattern" "$heading")"; then
      ...
      if [[ ! -s "$context_file" ]] \
        || ! grep -qx 'release_profile=testnet-0.2.14' "$context_file" \
        ...
        missing+=("$label PASS bundle for the current testnet-0.2.14 Genesis and model")
  ...
  if [[ "$label" == 'settlement evidence' ]]; then
    ...
    for candidate in "${settled[@]}"; do
      ...
      grep -qx 'release_profile=testnet-0.2.15' "$context_file" || continue
  ...
  if found="$(require_pass_bundle "$pattern" "$heading" "$evidence_name")"; then
    if [[ "$label" == 'v4 HA' || "$label" == 'Sepolia bridge' ]]; then
      context_file="$(dirname "$found")/context.env"
      if [[ ! -s "$context_file" ]] \
        || ! grep -qx 'release_profile=testnet-0.2.15' "$context_file" \
        || ! grep -qx "model=$MODEL_ID@$MODEL_REVISION" "$context_file" \
        || ! grep -qx "chain_id=$CHAIN_ID" "$context_file" \
        || ! grep -qx "genesis_sha256=$live_genesis_sha256" "$context_file" \
        || ! grep -qx 'devshard_version=v4' "$context_file"; then
        missing+=("$label PASS bundle for the current Genesis, testnet-0.2.15, current model, and v4")
        printf -- '- %s: STALE OR WRONG GENESIS/PROFILE (%s)\n' "$label" "$found" >>"$RUN/report.md"
        continue
      fi
    fi
...
if [[ -z "${GDC_SEPOLIA_CONTRACT:-}" || -z "${GDC_SEPOLIA_BEACON_STATE_URL:-}" ]]; then
  ...
  if bridge_runtime_bundle="$(require_pass_bundle '*-bridge-sepolia/*' '# Sepolia bridge: PASS')"; then
    runtime_dir="$(dirname "$bridge_runtime_bundle")"
    [[ -s "$runtime_dir/beacon-checkpoint.json" ]] \
      || missing+=("bridge runtime bundle lacks beacon preflight evidence")
    printf -- '- bridge runtime: PASS (%s)\n' "$bridge_runtime_bundle" >>"$RUN/report.md"
  else
    missing+=("Sepolia bridge runtime PASS")
    printf -- '- bridge runtime: BLOCKED (no epoch-move PASS bundle)\n' >>"$RUN/report.md"
  fi
fi
