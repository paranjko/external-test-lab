...
emit() {
  classification=$1 reason=$2 previous_profile_id=${3:-}
...
previous_profile_sha256="$(gdc_sha256 "$profile")"
current_profile_id="$(jq -r .profile_id "$current")"
last_state="$(printf '%s\n' "$chain" | jq -r .last_state)"
signer_started="$(printf '%s\n' "$chain" | jq -r .signer_ever_started)"
