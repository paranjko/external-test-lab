...
cmp -s "$legacy/inference/config/node_key.json" "$tmp/identity/p2p/node_key.json"
cmp -s "$legacy/tmkms/secrets/priv_validator_key.softsign" "$tmp/signer/tmkms/secrets/priv_validator_key.softsign"
