...
! grep -Fq './gdc.sh --release v2026.07.23 identities' "$ROOT/gdc-bash.sh"
! grep -Fq './gdc.sh --release v2026.07.23 identities' "$ROOT/README.md"
grep -Fq 'hosts=("$GENESIS_NODE")' "$ROOT/scripts/phase-qualify-ml.sh"
grep -Fq 'GDC_QUALIFY_HOSTS="$qualification_target"' "$ROOT/gdc-bash.sh"
grep -Fq 'qualify-ml ${host%-ml}' "$ROOT/scripts/lib.sh"
grep -Fq "genesis requires an SSH alias'" "$ROOT/gdc-bash.sh"
grep -Fq -- '--no-bootstrap-access' "$ROOT/gdc-bash.sh"
grep -Fq -- '--skip-qualification)' "$ROOT/gdc-bash.sh"
! grep -Fq -- '--sckip-qualification' "$ROOT/gdc-bash.sh"
grep -Fq 'GDC_GENESIS_SKIP_QUALIFICATION="$skip_qualification"' "$ROOT/gdc-bash.sh"
...
! grep -Fq 'gateway.join-client-key' "$ROOT/scripts/stage-network-bootstrap.sh"
...
bootstrap_line="$(grep -n 'Install the Genesis participant edge configuration' "$ROOT/scripts/phase-genesis.sh" | cut -d: -f1)"
synchronize_line="$(grep -n 'Refresh the public edge configuration after Genesis' "$ROOT/scripts/phase-genesis.sh" | cut -d: -f1)"
