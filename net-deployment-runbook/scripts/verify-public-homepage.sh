...
GRAFANA_INFERENCE_URL="${GDC_GRAFANA_INFERENCE_URL:-https://grafana.gonka-dev.net/d/gdc-inference/gonka-devnet-inference?orgId=1&from=now-24h&to=now&timezone=utc&kiosk}"
...
grep -Fq '<summary><span class="join-requirements-toggle">Minimum Host requirements</span> <span class="join-requirements-guide">For more details, read <a href="https://github.com/paranjko/external-test-lab/blob/main/net-deployment-runbook/ROLE-JOIN.md#join-add-a-host"' "$OUT/homepage.html"
! grep -Fq '<p>For more details, read' "$OUT/homepage.html"
node "$ROOT/scripts/verify-devnet-hardware-profile.mjs" \
  "$ROOT/profiles/devnet-hadware.json" "$OUT/homepage.html"
grep -Fq 'https://node0.gonka-dev.net/chain-api/productscience/inference/inference/models_all' "$OUT/homepage.html"
grep -Fq 'qualification loads the model, checks <code>/v1/models</code>, completes an inference request, and records VRAM evidence' "$OUT/homepage.html"
grep -Fq 'is not a guaranteed supported configuration</p>' "$OUT/homepage.html"
! grep -Fq 'is not a guaranteed supported configuration.</p>' "$OUT/homepage.html"
