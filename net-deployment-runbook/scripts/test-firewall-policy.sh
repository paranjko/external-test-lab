...
for log in "$tmp/rules.first" "$tmp/rules.second"; do
  ...
  grep -Fq -- '-s 198.51.100.30/32 -p tcp -m multiport --dports 8000,9099,18080,18085 -j ACCEPT' "$log"
  ! grep -Fq -- '--dports 3000,8000,8081,8082 -j ACCEPT' "$log"
...
decision() {
  ...
  if [[ "$source" == 198.51.100.10/32 && "$port" == 9101 ]] || \
     ...
     [[ "$source" == 198.51.100.30/32 && "$port" =~ ^(8000|1808[05])$ ]]; then
    printf 'ACCEPT\n'
  else
    printf 'DROP\n'
  fi
...
[[ "$(decision 198.51.100.30/32 8000)" == ACCEPT ]]
[[ "$(decision 203.0.113.44/32 8000)" == DROP ]]
