...
RUN="$ROOT/artifacts/runs/$(date -u +%Y%m%dT%H%M%SZ)-bridge-register-sepolia"
mkdir -p "$RUN"
...
curl -fsS https://node0.gonka-dev.net/v1/bridge/addresses >"$RUN/bridge-addresses-before.json" || true
...
curl -fsS https://node0.gonka-dev.net/v1/bridge/addresses >"$RUN/bridge-addresses-after.json"
