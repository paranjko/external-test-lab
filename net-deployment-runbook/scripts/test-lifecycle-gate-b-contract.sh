...
grep -Fq 'if [[ "$COMMAND" == confirmation-poc ]]; then' "$ROOT/gdc-bash.sh"
grep -Fq 'use_network_owner_data_home' "$ROOT/gdc-bash.sh"
...
grep -Fq 'CHAIN_BASE="${GDC_CHAIN_PUBLIC_BASE:-https://$PUBLIC_EDGE_HOST}"' "$ROOT/scripts/phase-confirmation-poc.sh"
