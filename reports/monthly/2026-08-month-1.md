...
- **Deployment runbook `1.0.0-alpha.0`** ([net-deployment-runbook/](../../net-deployment-runbook/)) — reproducible baseline on `testnet-0.2.14` (phased CLI: prepare → identities → qualify-ml → genesis → join → verify), governed `0.2.14 → 0.2.15` upgrade rehearsal, single-node stop/start/verify/reset, network reset contract. The M2 artifact "runbook sufficient to reproduce a node" ([JOIN.md](../../net-deployment-runbook/JOIN.md)) is already published.
