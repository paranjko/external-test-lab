...
## Current scoped decision – 2026-09-15

The initially unqualified profile
`environment/proposed-compatible-devshard-baseline-v10.json` has SHA-256
`5db7d1f1040e3c038ab97b1097a9bb53657d57814d4e5268bf59b7aa6673a497`.
Its exact owned, patched v5 composition was rendered from Gonka testenv source
`d5c27d83b0e22e46e483a5f89c8586c352aa9229` with composition digest
`bd625a9d05b9540b988e03d4155940d89bf16b522a3e4147f5592feb9c99be63`.
The running versiond child executed `/opt/versiond/bin/v5/devshardd` with
SHA-256 `3c1b8ddef0d3b8f17a37b4a9eaf2e1862c7a7b8407cb5e8920e5d606488a3f76`.
This is a local patched candidate, not the historical original pin or an
unmodified official release.

The profile-bound digest mismatch was rejected before launch or resource
creation. The matching owned fixture passed SG01–SG05, real non-stream and
SSE requests including `[DONE]`, malformed and missing-route rejection,
broken-mock non-success, and canary/SSE on a recreated gateway with isolated
mutable data. Its lease was released and terminal cleanup completed. A
separate independent Q2 review accepted only this exact v5 composition; its
decision ID is
`pr147-d7e14a6-v10-q2-independent-readonly-20260915`, bound to the frozen
review request SHA-256
`b2102b4d3126813b55b911960a40b90a102b354f67f995c26391670c26945bd2`.
The no-launch Q3 verifier accepted the decision and immutable evidence, with
decision SHA-256
`098ea743ef1a223246faddef73d2cb3bb969254a3ab8db8659ae4477c0cbbe40`.
The current verifier calls its derived result `qualified_for_acceptance`;
that means this precise backend qualification is eligible for the final M0
review, not whole-M0 acceptance. The original profile remains
`baseline.status=unqualified` as its historical input.

v3 and v4 are explicit unqualified contract gaps, not inferred compatible
adapters. The old original-pin `/devshard/v2/healthz` discrepancy remains
INCONCLUSIVE because its running binary and effective configuration were not
retained. The scoped decision does not qualify an unmodified release, shared
DevNet or other v5 compositions. Final exact-tree gates and fresh independent
all-row M0 acceptance are distinct pending checks.

The sections below retain earlier proposals, failed attempts and historical
receipt boundaries. Their “next run” language describes its original date
and is superseded by the current v10 result above; those outcomes are not
rewritten as PASS.
...
`environment/lab-mock-devshard-testenv-v5.json`.
...
`gonkactl-test --qualification-verify-only` verifies, but never creates, a separate
...
`environment/proposed-compatible-devshard-baseline-v1.json` is the initially
...
independent `gonkactl-test --preflight-only` decision binds that digest to the profile
...
`gonkactl-test` now requires `--profile` and validates it before composition
...
`gonkactl-test` writes that decision before invoking the real fixture command.
