...
func TestRunQualificationVerifyOnlyHasNoLaunchOrLease(t *testing.T) {
	dir := t.TempDir()
	profilePath := filepath.Join("..", "environment", "proposed-compatible-devshard-baseline-v8.json")
...
func TestRunPersistsProfileBeforeRejectingDigest(t *testing.T) {
	...
	err := run([]string{
		...
		"--profile", filepath.Join("..", "environment", "lab-mock-devshard-testenv-v5.json"),
		"false",
	})
...
func TestRunReleasesLeaseWhenSIGTERMStopsFixture(t *testing.T) {
	dir := t.TempDir()
	prepared := filepath.Join(dir, "prepared")
	if err := os.MkdirAll(prepared, 0o700); err != nil {
		t.Fatal(err)
	...
	probe, err := stand.PreflightComposition(prepared, "", filepath.Join(dir, "probe.json"))
	if err == nil || probe.ActualDigest == "" {
		t.Fatalf("digest precondition=%+v err=%v", probe, err)
	}
	receipt := filepath.Join(dir, "receipt", "decision.json")
	fixtureReceipt := filepath.Join(dir, "fixture.json")
	go func() {
	...
	err = run([]string{
		"--prepared-workdir", prepared,
		"--composition-digest", probe.ActualDigest,
		"--receipt", receipt,
		"--fixture-receipt", fixtureReceipt,
		"--lease-root", filepath.Join(dir, "leases"),
		"--environment-id", "lab-mock-devshard-testenv-v5",
		"--instance-id", "signal-cleanup",
		"--profile", filepath.Join("..", "environment", "lab-mock-devshard-testenv-v5.json"),
		"sh", "-c", "while :; do sleep 1; done",
	})
	if err == nil {
		t.Fatal("SIGTERM fixture launch unexpectedly succeeded")
	}
	contents, readErr := os.ReadFile(receipt)
	if readErr != nil {
		t.Fatal(readErr)
	}
	var decision stand.CompositionDecision
	...
	if decision.Lease == nil || !decision.Lease.Acquired || !decision.Lease.Released || decision.Outcome != "launch_failed" || decision.LaunchResult == "" {
		t.Fatalf("signal cleanup decision=%+v", decision)
	}
	if _, err := os.Stat(decision.Lease.Path); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("lease survives SIGTERM cleanup: %v", err)
	}
}
...
func TestRunPreflightOnlyAcceptsFrozenProposedBaselineWithoutLeaseOrLaunch(t *testing.T) {
	...
	err = run([]string{
		...
		"--profile", filepath.Join("..", "environment", "proposed-compatible-devshard-baseline-v1.json"),
	})
	if err != nil {
		t.Fatal(err)
	}
