// Package stand implements the managed-fixture adapter boundary.
package stand
...
func (p Profile) Validate() error {
	...
	if p.EnvironmentID == "" || p.EnvironmentKind != "lab-mock" {
		return fmt.Errorf("profile must name a lab-mock environment")
	}
	...
	for _, adapter := range p.Adapters {
		...
		if (adapter.Status == "candidate" || adapter.Status == "qualified") && (adapter.Health.Path == "" || adapter.Health.ExpectedStatus < 100) {
			return fmt.Errorf("%s %s lacks an executable health contract", adapter.Status, adapter.Protocol)
		}
		if (adapter.Status == "candidate" || adapter.Status == "qualified") && (adapter.Chat.Path == "" || adapter.Chat.NonStreamExpectedStatus < 100 || adapter.Chat.SSETermination == "" || adapter.Chat.MalformedExpectedStatus < 100) {
			return fmt.Errorf("%s %s lacks a complete chat contract", adapter.Status, adapter.Protocol)
		}
		if (adapter.Status == "candidate" || adapter.Status == "qualified") && (adapter.Negative.MissingRoutePath == "" || adapter.Negative.MissingRouteExpectedStatus < 100 || len(adapter.Negative.BrokenMockAcceptedOutcomes) == 0) {
			return fmt.Errorf("%s %s lacks a complete negative contract", adapter.Status, adapter.Protocol)
		}
	}
