...
func TestProfilePinsCandidateAndRefusesUnsupportedQualification(t *testing.T) {
	profile, err := LoadProfile(filepath.Join("..", "environment", "proposed-compatible-devshard-baseline-v8.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProposedBaselineProfileIsInitiallyUnqualifiedAndPinsV5Contract(t *testing.T) {
	...
	profile, err := LoadProfile(filepath.Join("..", "environment", "proposed-compatible-devshard-baseline-v1.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	v5 := profile.Adapters[2]
	if v5.Health.Path != "/v5/healthz" || v5.Chat.Path != "/v1/chat/completions" || v5.Negative.MissingRoutePath != "/v5/qualification-missing-route" {
		t.Fatalf("v5 compatibility contract=%+v", v5)
	}
}

func TestProfileRejectsUnexplainedUnsupportedAdapter(t *testing.T) {
	profile, err := LoadProfile(filepath.Join("..", "environment", "lab-mock-devshard-testenv-v5.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProfileRejectsQualifiedAdapterWithoutContracts(t *testing.T) {
	profile, err := LoadProfile(filepath.Join("..", "environment", "proposed-compatible-devshard-baseline-v8.json"))
	if err != nil {
		t.Fatal(err)
	}
	profile.Baseline.Status = "qualified"
	profile.Adapters[2].Status = "qualified"
	profile.Adapters[2].Chat.Path = ""
	if err := profile.Validate(); err == nil {
		t.Fatal("accepted qualified adapter without chat contract")
	}
}
