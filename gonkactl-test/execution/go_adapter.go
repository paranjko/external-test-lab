// Package execution runs executable test events without manufacturing Gherkin
...
func RunGoTest(ctx context.Context, directory, dataRoot string, selector GoTestSelector, extraArgs ...string) (GoTestReceipt, error) {
	...
	go func() { _, copyErr := io.Copy(&stderrBuffer, stderr); stderrDone <- copyErr }()
	events, parseErr := AdaptGoTestJSON(stdout, selector)
	waitErr := cmd.Wait()
	stderrErr := <-stderrDone
	receipt := GoTestReceipt{Selector: selector, Command: append([]string{"go"}, args...), StartedAt: started.UTC().Format(time.RFC3339Nano), Events: events, Stderr: stderrBuffer.String()}
	...
	if stderrErr != nil {
		return receipt, fmt.Errorf("capture go test stderr: %w", stderrErr)
	}
	if len(receipt.Events) == 0 {
