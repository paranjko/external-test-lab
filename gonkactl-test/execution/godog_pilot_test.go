...
func ownedPath(t *testing.T, name string) string {
	...
	p := filepath.Join(root, "execution-test", t.Name(), fmt.Sprintf("%d-%d", os.Getpid(), time.Now().UnixNano()), name)
...
func TestGodogPilotRecordsSchemaIdentityAndAllure(t *testing.T) {
	...
	rec := &runtimeRecorder{}
	opts := PilotOptions{RunID: "run-1", AttemptID: "attempt-1", FeaturePath: filepath.Join("..", "testdata", "feature", "pilot.feature"), JournalPath: journal, EvidenceDir: ownedPath(t, "evidence"), AllureRuntime: rec}
	...
	schema, err := compiler.Compile(filepath.Join("..", "schema", "event.schema.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestGodogPilotPersistsOfficialAllureSDKMessages(t *testing.T) {
	...
	if status := RunGodogPilotWithOptions(PilotOptions{
		RunID: "sdk-run", AttemptID: "sdk-attempt", FeaturePath: filepath.Join("..", "testdata", "feature", "pilot.feature"),
	...
	for i := 1; i <= 2; i++ {
		...
		go func() {
			...
			status := RunGodogPilotWithOptions(PilotOptions{
				...
				FeaturePath: filepath.Join("..", "testdata", "feature", "pilot.feature"),
...
func TestUndefinedAmbiguousPendingHookPanicAndCancellationStopExecution(t *testing.T) {
	cases := map[string]string{"undefined": "unknown step", "ambiguous": "ambiguous step", "pending": "pending step", "hook": "the test stand is prepared", "panic": "panic step", "cancel": "cancelled step"}
...
func TestRunnerJournalWriteFailureHasInterruptedReceipt(t *testing.T) {
	feature := filepath.Join("..", "testdata", "feature", "pilot.feature")
	journal := ownedPath(t, "events.jsonl")
...
func TestFixtureChangeFailsReachedAssertion(t *testing.T) {
	journal := ownedPath(t, "events.jsonl")
	status := RunGodogPilotWithOptions(PilotOptions{RunID: "run-broken", AttemptID: "attempt-broken", FeaturePath: filepath.Join("..", "testdata", "feature", "fixture_failure.feature"), JournalPath: journal, EvidenceDir: ownedPath(t, "evidence")})
	if status == 0 {
...
func TestGodogGivenFailureDoesNotFabricateLaterPass(t *testing.T) {
	journal := ownedPath(t, "events.jsonl")
	if status := RunGodogPilotWithOptions(PilotOptions{RunID: "run-given", AttemptID: "attempt-given", FeaturePath: filepath.Join("..", "testdata", "feature", "failure.feature"), JournalPath: journal, EvidenceDir: ownedPath(t, "evidence")}); status == 0 {
