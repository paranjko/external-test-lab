...
// ProduceArchive emits the six qualification records from the runner's journal. Keeping
...
func ProduceArchive(dir, runID, attemptID, feature string, events []JournalEvent) error {
	...
	manifest := map[string]any{"schema_version": "1.0.0", "run_id": runID, "parent_run_id": nil, "runner_build": "execution", "plan_hash": hex.EncodeToString(ph[:]), "immutable_target": map[string]any{"kind": "owned-local"}, "environment_instance_id": "execution-race", "lease_id": "local-only", "inventory_refs": map[string]any{"start": "inventory/start.json", "end": nil}, "execution_state": "complete", "gate_state": gateState, "report_state": reportState, "raw_exit": rawExit, "raw_signal": nil, "started_at": events[0].Timestamp, "finished_at": events[len(events)-1].Timestamp, "evidence_refs": []string{"events.jsonl"}}
...
func validateArchiveSchema(name string, value any) error {
	...
	c.AssertFormat()
	s, err := c.Compile(filepath.Join(filepath.Dir(source), "..", "schema", name+".schema.json"))
	if err != nil {
