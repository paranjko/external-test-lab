...
func RunGodogPilotWithOptions(opts PilotOptions) int {
	...
	suite := godog.TestSuite{ScenarioInitializer: func(sc *godog.ScenarioContext) {
		sc.Before(func(ctx context.Context, s *godog.Scenario) (context.Context, error) {
			...
			if strings.Contains(s.Name, "Ambiguous") || strings.Contains(s.Name, "Control") {
				for _, step := range s.Steps {
					if step.Text == "ambiguous step" {
						return ctx, errors.New("ambiguous binding: ambiguous step")
					}
				}
			}
			return ctx, nil
		...
		sc.Step(`^the test stand is prepared$`, func() error { return nil })
		sc.Step(`^the precondition is rejected$`, func() error { return errors.New("intentional given failure") })
		sc.Step(`^request "([^"]*)" is executed$`, func(ctx context.Context, v string) (context.Context, error) {
			state(ctx).Values["request"] = v
			return ctx, nil
		})
		sc.Step(`^the result is "([^"]*)"$`, func(ctx context.Context, want string) (context.Context, error) {
			got := "ok"
		...
		sc.Step(`^evidence contains "([^"]*)"$`, func(ctx context.Context, want string) (context.Context, error) {
			got := want
			return ctx, assertAndAttach(ctx, j, opts, "evidence", want, got)
		})
		sc.Step(`^a document is supplied$`, func(ctx context.Context, d *godog.DocString) (context.Context, error) {
			state(ctx).Values["document"] = d.Content
			return ctx, nil
		})
		sc.Step(`^the document contains "([^"]*)"$`, func(ctx context.Context, want string) (context.Context, error) {
		...
		sc.Step(`^a table is supplied$`, func(ctx context.Context, t *godog.Table) (context.Context, error) {
			var values []string
		...
		sc.Step(`^the table contains "([^"]*)"$`, func(ctx context.Context, want string) (context.Context, error) {
		...
		sc.Step(`^pending step$`, func() error { return godog.ErrPending })
		if opts.EnableAmbiguousDefinitions {
			...
			sc.Step(`^ambiguous step$`, func() error { return nil })
			sc.Step(`^ambiguous step$`, func() error { return nil })
		} else {
			sc.Step(`^ambiguous step$`, func() error { return errors.New("ambiguous control disabled") })
		}
		sc.Step(`^panic step$`, func() error { panic("controlled panic") })
		sc.Step(`^cancelled step$`, func() error { return context.Canceled })
