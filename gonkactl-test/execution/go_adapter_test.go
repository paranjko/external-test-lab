...
func TestRunGoTestKeepsStderrOutOfJSONStream(t *testing.T) {
	root := os.Getenv("GONKACTL_TEST_DATA_ROOT")
	receipt, err := RunGoTest(context.Background(), "..", root, selector("github.com/cucumber/godog/internal/models", "Test_Find/scenario"))
	if err != nil {
		t.Fatal(err)
	}
	if receipt.ExitCode != 0 || terminalCount(receipt.Events) != 1 {
		t.Fatalf("receipt=%+v", receipt)
	}
	// stderr is a separate diagnostic channel; it must not be parsed as a Go JSON event.
	for _, event := range receipt.Events {
		if strings.Contains(event.Test, "stderr") {
			t.Fatalf("stderr leaked into event: %+v", event)
		}
	}
}
