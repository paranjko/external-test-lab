...
func TestActualRunnerEmitsCompleteValidatedArchive(t *testing.T) {
	...
	if status := RunGodogPilotWithOptions(PilotOptions{RunID: runID, AttemptID: attemptID, FeaturePath: filepath.Join("..", "testdata", "feature", "pilot.feature"), JournalPath: journal, EvidenceDir: filepath.Join(dir, "evidence")}); status != 0 {
	...
	if err := ProduceArchive(dir, runID, attemptID, "testdata/feature/pilot.feature", events); err != nil {
		t.Fatal(err)
	}
