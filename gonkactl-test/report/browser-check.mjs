...
async function callGlobalFunction(call, functionDeclaration, values = []) {
  const global = await call("Runtime.evaluate", { expression: "globalThis" });
  ...
  if (result.exceptionDetails) throw new Error(`browser function failed: ${result.exceptionDetails.text}`);
  return result.result.value;
}

async function waitGlobalFunction(call, functionDeclaration, values, predicate, description) {
  const deadline = Date.now() + DOM_MS;
  let value;
  while (Date.now() < deadline) {
    value = await callGlobalFunction(call, functionDeclaration, values);
    if (predicate(value)) return value;
    await delay(100);
  }
  throw new Error(`rendered UI assertion deadline exceeded: ${description}`);
}

async function clickAttachment(call, label) {
  const point = await callGlobalFunction(call, `function(label) {
    const header = Array.from(document.querySelectorAll('[data-testid="test-result-attachment-header"]'))
      .find((item) => item.innerText.includes(label));
    const node = header?.closest('[data-testid="test-result-attachment"]')?.querySelector('button');
    if (!node) return null;
...
async function main() {
  ...
  try {
    ...
    if (options.probe) {
      ...
      const activated = await clickAttachment(call, wanted.attachmentLabel);
      ...
      const renderedAttachment = await waitGlobalFunction(call, `function(needle) {
        return Array.from(document.querySelectorAll('[data-testid="test-result-attachment-content"], [data-testid="test-result-attachment-text"], [data-testid="test-result-attachment-content-wrapper"], [data-testid="code-attachment-content"]'))
          .map((node) => node.innerText || node.textContent || '')
          .find((text) => text.includes(needle)) || '';
      }`, [wanted.attachmentNeedle], Boolean, "attachment content");
