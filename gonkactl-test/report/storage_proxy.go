...
func rewriteStorageAPIURL(body []byte, origin, prefix string) ([]byte, error) {
	...
	rewriteStorageURLs(&response, origin, prefix)
	rewritten, err := json.Marshal(response)
	if err != nil {
...
func rewriteStorageURLs(value *any, origin, prefix string) {
	switch current := (*value).(type) {
	case map[string]any:
		if target, ok := current["url"].(string); ok {
			if strings.HasPrefix(target, "/") {
				current["url"] = prefix + target
			} else {
				current["url"] = origin + prefix
			}
		}
		for key, child := range current {
			rewriteStorageURLs(&child, origin, prefix)
			current[key] = child
		}
	case []any:
		for i := range current {
