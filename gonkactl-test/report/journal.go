...
func readJournal(path string) ([]execution.JournalEvent, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	defer f.Close()
	var events []execution.JournalEvent
	s := bufio.NewScanner(f)
	s.Buffer(make([]byte, 65536), 4<<20)
	for s.Scan() {
		var e execution.JournalEvent
...
func convertEvents(events []execution.JournalEvent, journalPath, outputPath string, id Identity) error {
	r := allureResult{UUID: id.UUID, HistoryID: id.HistoryID, Name: id.Scenario, FullName: id.Feature + ":" + id.Scenario, Stage: "finished", Links: []any{}, Steps: []allureStep{}, Attachments: []allureAttachment{}, Labels: []allureLabel{{"epic", "Qualification"}, {"feature", id.Feature}, {"story", id.Scenario}, {"evidence_class", "authentic-runner-journal"}}}
...
func copyAttachment(journalPath, outputPath, ref string) (allureAttachment, error) {
	...
	source := strings.TrimSuffix(filepath.Base(outputPath), "-result.json") + "-" + hex.EncodeToString(digest[:8]) + "-" + name
	// The digest distinguishes colliding basenames; truncation makes a
	// re-conversion deterministic and replaces only its own derived artifact.
	out, err := os.OpenFile(filepath.Join(filepath.Dir(outputPath), source), os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return allureAttachment{}, err
	}
	_, cp := io.Copy(out, in)
...
func eventTimeMS(event execution.JournalEvent, roundUp bool) (int64, error) {
