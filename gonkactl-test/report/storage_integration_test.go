...
func TestStoragePrefixIntegration(t *testing.T) {
	...
	containerName := fmt.Sprintf("gonkactl-test-qualification-storage-%d", os.Getpid())
	...
	defer func() {
		...
		receipt := map[string]any{
			...
			"owner_label":                   "gonkactl-test.owner=qualification-storage-prefix",
	...
	bootstrapToken := "qualification-storage-bootstrap-only"
	secret := "qualification-storage-secret-only"
	if output, err := exec.Command(
		"docker", "run", "--detach", "--rm", "--name", containerName,
		"--label", "gonkactl-test.owner=qualification-storage-prefix",
	...
	reportID := fmt.Sprintf("qualification-storage-prefix-%d", os.Getpid())
...
func createStorageReport(t *testing.T, client *http.Client, proxyURL, token, reportID string) {
	t.Helper()
	payload := fmt.Sprintf(`{"repo":"gonkactl-test/lab-mock/qualification/v1","branch":"feat/gonkactl-test-qualification","reportUuid":%q,"name":"Qualification prefix smoke"}`, reportID)
...
func completeStorageReport(t *testing.T, client *http.Client, proxyURL, token, reportID string) {
	t.Helper()
	response := storageRequest(t, client, http.MethodPost, proxyURL+"/report/api/reports/"+reportID+"/complete", token, "application/json", strings.NewReader(`{"historyPoint":{"buildOrder":1,"reportName":"Qualification prefix smoke"}}`))
	body, _ := io.ReadAll(response.Body)
	_ = response.Body.Close()
...
func assertStoragePrefixSurface(t *testing.T, client *http.Client, proxyURL, reportID string, uploaded []string) {
	...
	body, _ := io.ReadAll(response.Body)
	_ = response.Body.Close()
	if response.StatusCode != http.StatusOK || !strings.Contains(string(body), "gonkactl-test authentic event qualification") {
		t.Fatalf("deep reload status=%d body=%q", response.StatusCode, body)
	}
	response, err = client.Get(proxyURL + "/report/reports/tree?repo=gonkactl-test/lab-mock/qualification/v1")
	if err != nil {
		t.Fatal(err)
	}
	body, _ = io.ReadAll(response.Body)
...
func assertStorageHistory(t *testing.T, client *http.Client, proxyURL, token string) {
	t.Helper()
	response := storageRequest(t, client, http.MethodGet, proxyURL+"/report/api/history?repo=gonkactl-test/lab-mock/qualification/v1&branch=feat/gonkactl-test-qualification", token, "", nil)
	body, _ := io.ReadAll(response.Body)
	_ = response.Body.Close()
