...
func TestSemanticHistoryIDCanonicalAndSeparated(t *testing.T) {
	...
	if want := hex.EncodeToString(sum[:]); got != want {
		t.Fatalf("HistoryID()=%s want %s", got, want)
	}
	changed := base
	...
	if got == other {
		t.Fatal("protocol variants collided")
	}
}
...
func TestValidateDAG(t *testing.T) {
	...
	if err := ValidateDAG(valid); err != nil {
		t.Fatal(err)
	}
	if got := BlockedBy(valid[2], map[string]bool{"session": false}); len(got) != 1 || got[0] != "session" {
...
func TestVerdictAndExitTable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.in.ExitCode(); got != tt.code {
				t.Fatalf("ExitCode=%d want %d", got, tt.code)
			}
			if got := tt.in.RunVerdict(); got != tt.verdict {
				t.Fatalf("RunVerdict=%s want %s", got, tt.verdict)
			}
		})
	}
}
